...
class TestGetMaxWeeks:
    ...
    def test_returns_13_for_2019_and_2020(self):
        """Test returns 13 weeks for 2019 and 2020 seasons (historical cap)."""
        from patriot_center_backend.utils.starters_loader import _get_max_weeks
        assert _get_max_weeks(season=2019, current_season=2024, current_week=10) == 13
        assert _get_max_weeks(season=2020, current_season=2024, current_week=10) == 13

    def test_returns_14_for_other_past_seasons(self):
        """Test returns 14 weeks for completed seasons (not 2019/2020)."""
        from patriot_center_backend.utils.starters_loader import _get_max_weeks
        assert _get_max_weeks(season=2021, current_season=2024, current_week=10) == 14
        assert _get_max_weeks(season=2023, current_season=2024, current_week=10) == 14
...
class TestFetchStartersForWeek:
    ...
    def test_fetches_from_sleeper_api_successfully(self, mock_fetch):
    ...
    @patch('patriot_center_backend.utils.starters_loader.LEAGUE_IDS', {2024: "league_123"})
    @patch('patriot_center_backend.utils.starters_loader.fetch_sleeper_data')
    def test_returns_empty_dict_on_matchups_api_failure(self, mock_fetch):
    ...
    @patch('patriot_center_backend.utils.starters_loader.fetch_sleeper_data')
    def test_applies_2019_early_week_cody_to_tommy_correction(self, mock_fetch):
    ...
    @patch('patriot_center_backend.utils.starters_loader.fetch_sleeper_data')
    def test_applies_2024_davey_roster_id_hardcode(self, mock_fetch):
    ...
    @patch('patriot_center_backend.utils.starters_loader.fetch_sleeper_data')
    def test_uses_unknown_manager_for_unmapped_display_names(self, mock_fetch):
...
class TestLoadOrUpdateStartersCache:
    ...
    def test_processes_2019_and_2020_with_13_week_cap(self, mock_fetch, mock_save, mock_load, mock_current):
        """Test processes 2019 and 2020 with 13-week cap."""
        from patriot_center_backend.utils.starters_loader import load_or_update_starters_cache

        mock_current.return_value = (2020, 13)
        mock_load.return_value = {}
        mock_fetch.return_value = {"Tommy": {"Total_Points": 100.0}}
